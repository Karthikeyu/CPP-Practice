...
const AuthProvider = ({ children }) => {
    ...
    const [authReadyUserId, setAuthReadyUserId] = useState(null); // To trigger dependent effects
    ...
    useEffect(() => {
        const attemptSignIn = async () => {
            try {
                ...
                console.error("Error during initial Firebase sign-in attempt:", error);
                if (auth.currentUser === null) {
                    try {
                        await signInAnonymously(auth); // Fallback
                    } catch (anonError) {
                        console.error("Error during fallback anonymous Firebase sign-in:", anonError);
                    }
                }
            }
        };

        attemptSignIn(); // Call attemptSignIn
        ...
        const unsubscribe = onAuthStateChanged(auth, (user) => {
            ...
            currentUserId = uId; // Set the global variable
            setAuthReadyUserId(uId); // Set state to trigger effects in children
            
            console.log("AuthProvider: currentUserId has been set to:", currentUserId); // DEBUG LOG
            
            initializeSampleData(); // Call after currentUserId is set and auth state is processed.
    ...
    const login = (username, password) => {
        if (username === "Franz" && password === "franz1234") {
            setIsAdmin(true);
    ...
    const logout = () => {
        setIsAdmin(false);
        localStorage.removeItem('isAdminLoggedIn');
        // For a real Firebase user admin, you'd also call auth.signOut()
        // window.location.hash = '#/admin/login'; // Handled by ProtectedAdminRoute or Navbar link
    };
...
const AdminLoginPage = () => {
    ...
    if (isAdmin) {
        // Use a timeout to allow React to finish rendering before hash change if needed,
        // though direct change is usually fine.
                ...
                <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
                    ...
                    <div className="rounded-md shadow-sm -space-y-px">
                        <div>
                            ...
                            <input id="username-admin" name="username" type="text" autoComplete="username" required 
